...
|-----------------------------------------------------------------------------------------------------------------|-----------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [Internal Monitor - Active Directory - Sync Out of Date](<../cwa/monitors/Active Directory - Sync Out of Date.md>)     | Internal Monitor | This monitor looks for the AD server that has been onboarded for more than 30 days and is experiencing a credential issue in the Active Directory Plugin.                                              |
| [Internal Monitor - Active Directory - Enabled Test Accounts](<../cwa/monitors/Active Directory - Enabled Test Accounts.md>) | Internal Monitor | This monitor will look for any account with the name **`Test`** in the account name and will flag that account if it is enabled on the domain.                                                         |
...
| [Internal Monitor - Active Directory - ADPluginUser - Create/Update](<../cwa/monitors/Active Directory - ADPluginUser - CreateUpdate.md>) | Internal Monitor | The purpose of this monitor set is to create an 'ADPluginUser' account for the domain controllers detected in AD Plugin.                                                                                 |
...
| [Script - New Domain Admin Monitor - Create](<../cwa/scripts/New Domain Admin Monitor - Create.md>)                 | Client Script    | Creates remote monitors to ticket when a new domain admin is added and has not been excluded.                                                                                                         |
| [Active Directory Reporting Solution](<../cwa/reports/Active Directory Reporting Solution.md>)                         | Reporting Solution | Full Reporting solution that pulls from the Active Directory Plugin                                                                                                                                     |
...
| Content                                                                                                         | Type            | Function                                                                                                                                                                                                 |
|-----------------------------------------------------------------------------------------------------------------|-----------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [Internal Monitor - Active Directory - User Last Logon > X Days](<../cwa/monitors/Active Directory - User Last Logon  X Days.md>) | Internal Monitor | This monitor will check for users who have not logged in for more than X days and are not administrator accounts.                                                                                      |
| [Internal Monitor - Active Computers in AD with No Agent](<../cwa/monitors/Active Computers in AD with No Agent.md>)    | Internal Monitor | The monitor set generates a client-level ticket containing details of domain-joined computers active within the domain, which have been joined to the domain for at least 7 days but do not have the Automate agent installed. |
...
2. Ensure that the RMM+ Plugin is correctly configured and operational within the environment, as this solution relies on the proper configuration of the [CWM - Automate - RMM+ Plugin Configuration](<../cwa/scripts/CWM - Automate - RMM+ Plugin Configuration.md>).
...
3. Ensure the following content is imported to the environment as it is used in multiple solutions for ticketing:
   ...
   - [Script - Ticket Creation - Client](<../cwa/scripts/CWM - Automate - Ticket Creation - Client.md>)
...
1. Import the following content using the ProSync plugin:
   - [Internal Monitor - Active Directory - Sync Out of Date](<../cwa/monitors/Active Directory - Sync Out of Date.md>)
   - [Script - ScreenConnect - RMM+ Autofix - AD Plugin - Sync Out of Date [Ticket]](<../cwa/scripts/ScreenConnect - RMM+ Autofix - AD Plugin - Sync Out of Date Ticket.md>)
...
3. Navigate to Automation → Monitors → Internal Monitors and configure the following:
   - Find the [Internal Monitor - Active Directory - Sync Out of Date](<../cwa/monitors/Active Directory - Sync Out of Date.md>).
...
1. Import the following using the ProSync Plugin:
   - [Internal Monitor - Active Directory - Enabled Test Accounts](<../cwa/monitors/Active Directory - Enabled Test Accounts.md>)

2. Reload the System Cache:
   ![Image](../../static/img/Active-Directory-Monitoring-Solution/image_5.png)

3. Navigate to Automation → Monitors → Internal Monitors and configure the following:
   - Find the [Internal Monitor - Active Directory - Enabled Test Accounts](<../cwa/monitors/Active Directory - Enabled Test Accounts.md>).
...
1. Import the following content from the ProSync Plugin:
   - [Internal Monitor - Domain Computers Missing Automate Agent](<../cwa/monitors/Active Computers in AD with No Agent.md>)
...
1. Import the following content using the ProSync Plugin:
   - [Internal Monitor - Active Directory - ADPluginUser - Create/Update](<../cwa/monitors/Active Directory - ADPluginUser - CreateUpdate.md>)
   - [Script - Active Directory - Plugin User Account - Create/Update](<../cwa/scripts/Active Directory - Plugin User Account - CreateUpdate.md>)
...
1. Import the following monitor using the ProSync Plugin:
   - [Internal Monitor - Active Directory - User Last Logon > X Days](<../cwa/monitors/Active Directory - User Last Logon  X Days.md>)
...
4. Navigate to Automation → Monitors → Internal Monitors and configure the following:
   - Find the [Internal Monitor - Active Directory - User Last Logon > X Days](<../cwa/monitors/Active Directory - User Last Logon  X Days.md>).
     - Assign the **`△ Custom - Ticket Creation - Computer`** alert template.
     - OR
     - Assign the **`△ Custom - Email Creation - Computer`** alert template.
...
1. Import the following content from the ProSync Plugin:
   - [Script - Group Policy - Audit](<../cwa/scripts/Group Policy - Audit.md>)
   - [Dataview - Windows - Group Policy Objects](<../cwa/dataviews/Windows - Group Policy Objects.md>)
   - [Dataview - Windows - Group Policy Settings](<../cwa/dataviews/Windows - Group Policy Settings.md>)
   - [Dataview - Windows - Group Policy Object Links](<../cwa/dataviews/Windows - Group Policy Object Links.md>)
   - [Dataview - Windows - Group Policy Object Security Filtering](<../cwa/dataviews/Windows - Group Policy Object Security Filtering.md>)
   - [Internal Monitor - GPO Modified](<../cwa/monitors/GPO Modified.md>)
     - *Note: This is optional. Only import this if specifically requested.*

2. Reload the System Cache:
   ![Image](../../static/img/Active-Directory-Monitoring-Solution/image_5.png)

3. Schedule the [Script - Group Policy - Audit](<../cwa/scripts/Group Policy - Audit.md>) to run once per day against the domain controllers group.
...
4. **Only perform this step if requested by the consultant:**
   - Navigate to Automation → Monitors → Internal Monitors and configure the following:
     - Find the [Internal Monitor - GPO Modified](<../cwa/monitors/GPO Modified.md>).
       - Assign the **`△ Custom - Ticket Creation - Computer`** alert template.
       - OR
       - Assign the **`△ Custom - Email Creation - Computer`** alert template.
...
1. Import the following content from the ProSync Plugin:
   - [Script - AD - Enable AD Recycle Bin](<../cwa/scripts/AD - Enable AD Recycle Bin.md>)
   - Alert Template - **`△ Custom - Execute Script - AD - Enable AD Recycle Bin`**

2. Import the [Role - AD Domain Recycle Bin Feature](<../cwa/roles/AD Domain Recycle Bin Feature.md>).

3. Import the [CWM - Automate - Remote Monitor - AD Recycle Bin State Check](<../cwa/monitors/AD Recycle Bin State Check.md>) using the following instructions outlined here: [Import - Remote Monitor - AD Recycle Bin State Check](<../cwa/monitors/AD Recycle Bin State Check.md>).

4. Reload the System Cache:
   ![Image](../../static/img/Active-Directory-Monitoring-Solution/image_5.png)

5. Locate the [EPM - User Management - Remote Monitor - Reset AD Users Password Age](<../cwa/monitors/Reset AD Users Password Age.md>) remote monitor.
...
1. Import the [Remote Monitor - Active Directory Replication Anomaly Monitoring](<../cwa/monitors/Active Directory Replication Anomaly Monitoring.md>) using the following instructions outlined here: [Implement - Remote Monitor - Active Directory Replication Anomaly Monitoring](<../cwa/monitors/Active Directory Replication Anomaly Monitoring.md>).

2. Reload the System Cache:
   ![Image](../../static/img/Active-Directory-Monitoring-Solution/image_5.png)

3. Locate the [RSM - Active Directory - Remote Monitor - Active Directory Replication Anomaly Monitoring](<../cwa/monitors/Active Directory Replication Anomaly Monitoring.md>) remote monitor.
   - By opening the **`Domain Controllers`** group.
...
1. Import the [Remote Monitor - Domain Admin Account Lockout](https://proval.itglue.com/5078775/docs/16951465) using the following implementation instructions:
   - [Implement - Remote Monitor - Domain Admin Account Lockout](<../cwa/monitors/Implement - Remote Monitor - Domain Admin Account Lockout.md>).
