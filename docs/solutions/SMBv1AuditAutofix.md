...
tags: ['audit', 'monitor', 'ticketing', 'vulnerability', 'windows']
draft: false
unlisted: false
---
## Purpose
...
|-----------------------------------------------------------------------------------------------------------------------|--------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [Script - SMBv1 Status Audit/Autofix [DV,Param]](https://proval.itglue.com/DOC-5078775-15811522)                   | Script       | This script is created to run the detection/remediation for SMBv1 Enabled based on EDF. It will act only if the dropdown EDF "SMB1 Vulnerability" is set to 'Audit Only' or 'Audit With Autofix' at any client, location, or computer level.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| [Script - SMBv1 Enable Detection Ticketing - Per Client](https://proval.itglue.com/DOC-5078775-17229170)           | Client Script| This client script creates a ticket for each client with the computer count where remediation is required.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| [Script - SMB1 Traffic Detection Monitor Creation](https://proval.itglue.com/DOC-5078775-17454586)                  | Client Script| Remote monitors "ProVal - Production - SMB1 Traffic Detection" will be set up on computers where the [SMB1 Detection](https://proval.itglue.com/DOC-5078775-13177312) remote monitor identifies the SMB1 Server Status as `Enabled`. In the [Dataview - SMBv1 Status Audit](https://proval.itglue.com/DOC-5078775-15811524), the `SMB1 Server Status` column will display `Enabled` for these machines. [Dataview - SMBv1 Status Audit](https://proval.itglue.com/DOC-5078775-15811524) gathers the state of the monitor failure to show the machines where the SMB1 is used.                                                                                                                                                                                                                                                 |
| [Dataview - SMBv1 Status Audit](https://proval.itglue.com/DOC-5078775-15811524)                                    | Dataview     | This dataview gathers the data from the script [CWA Script - SMBv1 Enabled Detection & Remediation](https://proval.itglue.com/DOC-5078775-15811522) and depicts the status of SMB Enabled on the agent where it is deployed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| [Remote Monitor - SMB1 Detection](https://proval.itglue.com/DOC-5078775-17241874)                                   | Remote Monitor| Check whether the SMB1 is enabled on the end machine or not. It runs the OS version [compatible command](https://learn.microsoft.com/en-us/windows-server/storage/file-server/troubleshoot/detect-enable-and-disable-smbv1-v2-v3?tabs=server#how-to-detect-status-enable-and-disable-smb-protocols) to fetch the data.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| [Internal Monitor - SMB1 Autofix](https://proval.itglue.com/DOC-5078775-18081997)                                   | Internal Monitor| This internal monitor detects online Windows agents and performs autofix on computers where the "Audit with Autofix" is set via client, location, or computer level EDF "SMB1 Vulnerability". The [Remote Monitor - SMB1 Detection](https://proval.itglue.com/DOC-5078775-17241874) shows failure, and the autofix [Script - SMBv1 Status Audit/Autofix [DV, Param]](https://proval.itglue.com/DOC-5078775-15811522) has not run in the past week.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| **△ Custom - Execute Script - SMB1 Audit/Autofix**                                                                   | Alert Template| This needed to be scheduled with the [Remote Monitor - SMB1 Detection](https://proval.itglue.com/DOC-5078775-17241874) and the [Internal Monitor - SMB1 Autofix](https://proval.itglue.com/DOC-5078775-18081997) to perform the Audit and Autofix based on EDF selection at the client, location, or computer level.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
- [EPM - Windows Configuration - Dataview - SMB1 Status [Remote Monitor]](https://proval.itglue.com/DOC-5078775-8202422)  
...
1. Import the following content using the ProSync Plugin:
   ...
   - [Script - SMBv1 Enable Detection Ticketing - Per Client](https://proval.itglue.com/DOC-5078775-17229170)  
   - [Script - SMB1 Traffic Detection Monitor Creation](https://proval.itglue.com/DOC-5078775-17454586)  
   - [Dataview - SMBv1 Status Audit](https://proval.itglue.com/DOC-5078775-15811524)  
   - [Internal Monitor - SMB1 Autofix](https://proval.itglue.com/DOC-5078775-18081997)  
...
3. Configure the solution as outlined below:
   ...
   - Schedule the client script "[Script - SMB1 Traffic Detection Monitor Creation](https://proval.itglue.com/DOC-5078775-17454586)" to gather the SMB1 live used state from the machines based on the event ID detection.
     - This client script will deploy the remote monitor to all the computers where "[Remote Monitor - SMB1 Detection](https://proval.itglue.com/DOC-5078775-17241874)" has detected SMB1 enabled.
     - The [Dataview - SMBv1 Status Audit](https://proval.itglue.com/DOC-5078775-15811524) gathers the state of the monitor "ProVal - Production - SMB1 Traffic Detection" and shows data as "Yes" or "No" if used or not used respectively.
