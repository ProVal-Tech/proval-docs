...
tags: ['windows', 'update', 'configuration', 'performance', 'recovery']
draft: false
unlisted: false
---
## Summary
...
3. The download process begins.
   1. A check is performed to confirm if the ZIP file already exists on the target machine. This is done through MD5 hash validation. If it does exist, then the download is skipped.
...
8. If the update is found to be successful, then the script notes this in the script log and exits.
9. If the HoursToWait is exceeded and the update is not complete, then the script will attempt to gather logs by downloading SetupDiag.exe from Microsoft and write these to the script log for further troubleshooting.
