...
tags: ['cleanup', 'logging', 'scripts', 'database', 'maintenance', 'connectwise']
draft: false
unlisted: false
---
## Summary
...
### Changes:
1. Truncate `windowsupdateetlfiles` table and [disables the etl files collection](https://connectwise20.my.site.com/serviceandsupport/s/article/Disable-Windows-10-Log-File-collection-to-relieve-web-congestion).
...
- It will set the system properties to the default value as shown in the below screenshot. You can change the values of these parameters as per the client's requirement by modifying the system properties directly.
  ![System Properties Screenshot](5078775/docs/11637293/images/20274025)
- Open your system dashboard and navigate to management / Scheduled Client Scripts.
  ![Scheduled Client Scripts Screenshot](5078775/docs/11637293/images/16255049)
...
| Name              | Description                                                                                                                                                                                                                                                                     |
|-------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| LogMessage         | This is a running Log variable that holds the entirety of the Txt file that is used in logging. This variable should be utilized in each sub script to add to this variable and keep the running log. All instances of this beyond the first should be `@logmessage@ = @logmessage@ [add something one line down] |
| ComputerID         | The computer ID of the target machine to upload the log to. Selects the automate server or the first domain controller in the environment by ID.                                                                                                                                 |

## System Properties

| Name                                         | Example                                | Required | Description                                                                                                                                                                                                                             |
|----------------------------------------------|----------------------------------------|----------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| DBCleanup_Ticket_Days                        | 0 or blank for disabled                | False    | The time in day ago that you would like all tickets removed if created before. By default its value will be '60' days.                                                                                                             |
...
| DBCleanup_Inactiveagreementmapping           | 0 or blank to disable                  | False    | This toggles the inactive agreement mappings cleanup for CW Manage plugin. By Default its value will be '1'.                                                                                                                         |
| DBCleanup_UserClass_Enable_Default_Permission | 0 to disable                            | False    | Set it to "1" to enable sync of userclass permissions from Default Client to all other clients. By Default its value will be '0'.                                                                                                    |
| DBCleanup_UserClass_Default_Clientid         | 0 to disable                            | False    | Mention the client's clientid from which you would like to copy the permissions to other clients. By Default its value will be '0'.                                                                                                   |
| DBCleanup_UserClass_Excluded_Clients         | 0 to disable                            | False    | Mention the clientid's you would like to exclude from getting the default user class permissions. By Default its value will be '0'.                                                                                                  |

## User Parameter

| Name              | Example | Required                                         | Description                                                                                                                                                                                                                       |
|-------------------|---------|--------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Set_Environment    | 1       | Need to set to 1 for first-time implementation   | Running the script with the user parameter `Set_Environment` as `1` will import System Properties needed for the script.                                                                                                        |
...
![Expected Log File Screenshot](5078775/docs/11637293/images/17472317)
