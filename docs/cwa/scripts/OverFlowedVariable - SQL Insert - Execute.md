...
tags: ['sql', 'json', 'optimization', 'script', 'database']
draft: false
unlisted: false
---
## Summary
...
| `@LimitedOverFlowResults@` | Script           | The results of the PowerShell that formats the SQL statement to its most optimized for space limits.                                                    |
| `@TotalRowsToWrite@`    | Script           | This variable is created when splitting `@LimitedOverFlowResults@`, it holds the amount of datapoints in the Json file.                                   |
| `@TotalRowsToLimit@`    | Script           | This variable is created when splitting `@LimitedOverFlowResults@`, it holds the amount of datapoints Automate can handle before exceeding the variable space limit of Automate. |
| `@SQLInsert@`           | Script           | This variable is created when splitting `@LimitedOverFlowResults@`, it holds the limited SQL insert statement to be executed.                             |
...
3. For every data point in the json file
   - Get every Datapoint Name and populate `"'$($DataPoint.($DataPointName))'"}) -join ',' Count each Datapoint's Character Count, and return the Average Data point count
