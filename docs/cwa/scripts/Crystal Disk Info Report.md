...
tags: ['disk', 'monitoring', 'ticket', 'automation', 'windows', 'health']
draft: false
unlisted: false
---
## Summary

The script runs the [crystal disk info tool](https://crystalmark.info/en/software/crystaldiskinfo/) on the physical machines, stores the basic disk info returned by the tool in a [custom table](https://proval.itglue.com/DOC-5078775-12960038), and creates a ticket if the [Health Status](https://crystalmark.info/en/software/crystaldiskinfo/crystaldiskinfo-health-status/) returned by the tool does not contain `Good`. Also, it attempts to attach the `DiskInfo.txt` report generated by the tool to the ticket.

It is an Automate implementation of the agnostic script [EPM - Disk - Agnostic - Get-CrystalDiskInfo](https://proval.itglue.com/DOC-5078775-12918412).

Note: The script will not work for virtual machines as the tool only supports physical hard disks.

## Sample Run

![Sample Run](5078775/docs/12960045/images/18505064)
...
- [Crystal Disk Info - Ticket Troubleshooting Guide](https://proval.itglue.com/DOC-5078775-14394330)
...
| TicketCreationCategory  | Id of the ticket category of the ticket to generate                                                       |
| TicketVariable          | It contains four variables: ExistingTicketId, TicketSubject, TicketBody, InternalNotes                   |
| ExistiningTicketID      | Ticketid of already open ticket for the computer.                                                         |
| TicketSubject           | Subject of ticket to create                                                                                |
...
![Extra Data Fields](5078775/docs/12960045/images/20792237)

## System Properties

| Name                             | Default | Required | Description                                                                                 |
|----------------------------------|---------|----------|---------------------------------------------------------------------------------------------|
| Crystal_Disk_Info_Disable_Tickets| 0       | False    | 0 or 1 to toggle ticket creation function of the script.                                   |
| Crystal_Disk_Info_Disable_Caution_Tickets| 0 | False | 0 or 1 to toggle ticket creation for the disks with the `Caution` Health Status.          |
...
```
Disk(s) information returned by the tool are outlined below (Please refer to ticket's internal notes for further details):
Model: <Disk 1 Model>
...
If Script fails to find the file:  
```
...
If Script finds and uploads the file:  
```
...
![Sample Ticket Summary](5078775/docs/12960045/images/18506108)

**Internal Notes/Comment:**  
![Sample Internal Notes](5078775/docs/12960045/images/18506155)

Attached **`DiskInfo.txt`** File:  
![Attached DiskInfo.txt](5078775/docs/12960045/images/18506200)

## Excluding Disk(s) from alerting

- Gather the serial number of disk(s) to exclude from the generated ticket.  
...
e.g.,  
![Example Exclusion](5078775/docs/12960045/images/19197615)
