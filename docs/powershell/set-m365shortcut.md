...
description: 'Creates shortcuts for installed Microsoft 365 applications directly on the Public Desktop (or in a specified folder), waiting for the Office install to finish first.'
...
This script places a shortcut for every installed Microsoft 365 application directly on the **Public Desktop**. Optionally, when a `-FolderName` is provided, it gathers the shortcuts into a single folder of that name instead.

Because the shortcuts (or folder) live on the Public Desktop, they appear on the desktop of **every user** of that machine, including users who log in for the first time later. That removes the per-user shortcut step from a build or onboarding process.
...
- **Idempotent.** Existing shortcuts are rewritten to point at the current executables, and a named folder is reused if it already exists — so it is safe to re-run after an Office repair, reinstall, or version change.
...
Waits up to 45 minutes for Microsoft 365 to finish installing, then creates a shortcut for every installed application directly on `C:\Users\Public\Desktop`.

To group the shortcuts into a folder instead of placing them directly on the desktop:

```powershell
.\Set-M365Shortcut.ps1 -FolderName 'Microsoft 365'
```
...
3. Places the shortcuts directly on the Public Desktop, or creates a named folder to hold them if `-FolderName` is specified.
...
The shortcuts become visible on the desktop of every user of the machine. Existing user desktops are otherwise untouched — no per-user shortcuts are added, moved, or removed.
...
- Does **not** remove shortcuts, and does **not** clean up the folder or desktop if an application is later uninstalled.
- Does **not** pin the shortcuts or folder, set custom icons, or control where they appear on the desktop.
...
4. **Create the shortcuts** — If a `-FolderName` is provided, the folder is created if missing. Each shortcut is written, then confirmed to exist on disk before being reported as successful.
...
Waits up to 45 minutes, then places a shortcut for every installed application directly on `C:\Users\Public\Desktop`.

```powershell
.\Set-M365Shortcut.ps1 -FolderName 'Microsoft 365'
```

Waits for the install, then groups the shortcuts into `C:\Users\Public\Desktop\Microsoft 365` instead of placing them directly on the desktop.

```powershell
.\Set-M365Shortcut.ps1 -FolderName 'Office Apps' -TimeoutMinutes 90
```

Uses `C:\Users\Public\Desktop\Office Apps` to hold the shortcuts, and allows the install up to 90 minutes to finish. Useful on slow links where a full Microsoft 365 download takes longer than the default allowance.
...
| `FolderName`      |       | False    | *(None)*          | String     | The name of a folder to create on the Public Desktop to hold the shortcuts. When omitted, shortcuts are placed directly on the Public Desktop. A folder name only — path separators and other invalid file name characters are rejected. |
...
```text
C:\Users\Public\Desktop\<Application>.lnk
...
| `Failed to create the Public Desktop folder '<path>'.` | Insufficient rights, or the path is blocked by a policy or security product (only occurs if `-FolderName` is used). | Confirm the script ran elevated (as `SYSTEM` from the RMM). Check for a policy restricting writes to the Public Desktop. |
...
| --- | --- | --- |
| Shortcuts placed directly on the Public Desktop | No | Delete the individual `.lnk` files from `C:\Users\Public\Desktop`. |
| Folder created on the Public Desktop (if `-FolderName` was used) | No | `Remove-Item -Path 'C:\Users\Public\Desktop\<FolderName>' -Recurse -Force` |
...
Deleting the folder (if created) removes the grouped shortcuts from every user's desktop at once. If shortcuts were placed directly on the desktop, they must be deleted individually. Individual users can also delete the items from their own desktop only if they have rights to the Public Desktop — normally they cannot, which is the point.
...
- Shortcuts are now placed directly on the Public Desktop by default. The `-FolderName` parameter is now optional and only creates a grouping folder when explicitly provided.
