...
| ----- | ---- | ---------------- | ---- | -------- | ------------- | --------------------- | --------------------- | -------------- | ----------- | -------- | ----------- | ----------- |
| cpvalWindowsKekCertificate | cPVAL Windows KEK Certificate | `Device` | `Text` | False | -- | `Editable` | `Read/Write` | `Read/Write` | This custom field displays the status of the Windows Key Exchange Key (KEK) certificate. | Shows whether the KEK certificate is installed and valid for secure boot updates. | Important for secure boot updates and key management. | SecureBoot Status |
