...
Choose OS to enforce TLS hardening for Workstations and Servers. Select `Disable` to disable TLS hardening enforcement.

## Details

| Label | Field Name | Definition Scope | Type | Available Options |  Default Value | Technician Permission | Automation Permission | API Permission |Custom Field Tab Name |
| ----- | ---- | ---------------- | ---- | -------- | ------------- | --------------------- | --------------------- | -------------- | ----------- | 
| cPVAL TLS Hardening | cpvalTlsHardening | `Organization`, `Location`, `Device` | DropDown | `Windows Devices`, `Windows Servers`, `Windows Workstations`,`Disable` | - | Editable | Read/Write | Read/Write |  TLS/SSL |
