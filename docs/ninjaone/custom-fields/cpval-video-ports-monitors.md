...
| ----- | ---- | ---------------- | ---- | -------- | ------------- | --------------------- | --------------------- | -------------- | ----------- | -------- | ----------- |
|cPVAL Video Ports Monitors | cpvalvideoportsmonitors | Device | multi-line | `Windows`, `desktops`, `and`, `laptops` | `Disabled` | `Editable` | `Read/Write | `Read/Write` | 'This custom field is used show available video output ports on the system (HDMI, DisplayPort, VGA, DVI, etc.). Detect which ports are actively used by connected monitors. | Video Ports | Video Ports |
