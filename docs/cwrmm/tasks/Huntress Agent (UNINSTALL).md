...
tags: ['uninstallation', 'windows', 'software', 'script', 'configuration']
draft: false
unlisted: false
---
## Summary

Attempts to uninstall the Huntress agent on the Windows machine if the Huntress agent is installed on it.
...
- Add another row by selecting `ADD ROW` button in the `Else` section.
- Search and select the `Script Exit` function.
- Input the following:
