...
The "ImmyBot Installer Key" is a specific identifier used during the installation of the ImmyBot agent. It's a unique string that associates the agent with your ImmyBot tenant and is used installing the agent. You can find it within the installation script generated by ImmyBot for your tenant.
...
| ---- | ----- | ---- | ------------- | -------- | ----------- |
| ImmyBot Installer Key | COMPANY | Text Box | | Yes | TThe installer key string required to activate the installer. Must be correctly configured to install the agent. |
